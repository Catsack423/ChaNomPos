<x-app-layout>
    ...
    <div class="grid stockcols">
        ...
        <div class="left-column">
            ...
            <div class="card"
                style="background: white; padding: 20px; border-radius: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin-bottom: 20px;">
            ...
            <div class="stock-container">
                ...
                <div class="section-card"
                    ...
                    <div class="section-title"
                        ...
                        <span style="font-weight: bold; font-size: 1.1rem;">กำลังใช้งาน <span
                                style="color:#8bc34a; font-size:0.8rem; font-weight:normal;">(เปิดถังแล้ว)</span></span>
                    ...
                    <table class="figma-table" style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr
                                style="text-align: left; color: #888; font-size: 0.85rem; border-bottom: 1px solid #eee;">
                        ...
                        <tbody>
                            @if (isset($activeLots) && $activeLots->count() > 0)
                                @foreach ($activeLots as $lot)
                                    ...
                                    <tr id="lot-row-{{ $lot->id }}" class="active-lot-row"
                                        ...
                                        <td>
                                            <div
                                                style="display: flex; justify-content: space-between; font-weight: bold; font-size: 0.8rem; margin-bottom: 4px;">
                                                <span>{{ number_format(max($remain, 0), 2) }}</span>
                                                <span>{{ $percent }}%</span>
                                            </div>
                                            <div
                                                style="background: #eee; height: 6px; border-radius: 10px; overflow: hidden;">
                                                <div
                                                    style="width: {{ $percent }}%; background: {{ $progressColor }}; height: 100%;">
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            @else
                                <tr>
                                    <td colspan="5" class="text-center" style="padding:20px; color:#ccc;">
                                        ไม่มีวัตถุดิบที่กำลังเปิดใช้งาน</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
                    ...
                    <table style="width:100%;border-collapse:collapse;">
                        ...
                        @foreach ($stockLots as $lot)
                            <tr style="border-bottom:1px solid #f9f9f9;">
                                ...
                                <td style="width:45px;padding:10px 5px;">
                                    <img src="{{ $lot->imgurl ? asset('img/' . $lot->imgurl) : asset('img/logo.png') }}"
                                        width="35" height="35" style="border-radius:8px;object-fit:cover;">
                                </td>
                ...
                <form action="{{ route('admin.stock.add') }}" method="POST">
                    ...
                    <div style="margin-bottom: 15px;">
                        <input type="text" name="name" class="input"
                            placeholder="ชื่อวัตถุดิบ (เช่น นมข้น, ผงชา)" required
                            style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;">
                    </div>
                    <div style="margin-bottom: 15px;">
                        <input type="text" name="unit" class="input" placeholder="หน่วย (เช่น ml, g, ชิ้น)"
                            required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;">
                    </div>
                    <button type="submit"
                        style="width: 100%; background: #7B4A2E; color: white; border: none; padding: 12px; border-radius: 10px; cursor: pointer; font-weight: bold;">สร้างประเภทวัตถุดิบ</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="grid logcols" style="margin-top: 30px;">
        <div class="card"
            ...
            <h2 style="margin-bottom: 20px; font-size: 1.1rem; color: #000; font-weight: bold">ประวัติการใช้สต็อกล่าสุด
            </h2>
            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                <table class="figma-table" style="width: 100%; border-collapse: collapse;">
                    <thead style="position: sticky; top: 0; background: white; z-index: 1;">
                        <tr
                            style="text-align: left; color: #888; font-size: 0.85rem; border-bottom: 2px solid #fdfaf8;">
                    ...
                    <tbody>
                        ...
                        @foreach ($logs as $log)
                            @php
                                // เปลี่ยนมาใช้ ?-> เพื่อป้องกันระบบพังเมื่อไม่พบล็อตสินค้า
                                $ingredientName = $log->real_ingredient?->ingredient?->name ?? 'ไม่พบ (วัตถุดิบถูกลบ)';
                            @endphp
        ...
        <div class="modal-box">
            ...
            <form id="addLotForm" enctype="multipart/form-data">
                ...
                <div class="upload-box">
                    ...
                    <img id="previewImage" src="{{ asset('img/logo.png') }}" class="preview-img">
